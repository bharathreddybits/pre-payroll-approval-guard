...
export const fundamentalPayRules: RuleDefinition[] = [
  ...
  },
  {
    ...
    minTier: 'starter',
    flagReason: 'Net pay exceeds gross pay, which is mathematically impossible after taxes and deductions.',
    riskStatement: 'Gross pay less than net pay is a mathematical impossibility. Any downstream 941 filing, W-2, or ACH processor will produce invalid results. This must be corrected before approval.',
    commonCauses: [
      'Gross and net columns swapped in the export file',
      'Negative taxes or deductions applied (deduction entered as negative number)',
      'Expense reimbursement added to net pay but not reflected in gross',
      'Data import mapping error inverting the values',
    ],
    reviewSteps: [
      'Verify gross and net pay values are in the correct columns',
      'Check for negative tax or deduction values that would inflate net pay',
      'Ensure reimbursements are properly classified as gross earnings',
      'Correct the values in your payroll system and re-upload',
    ],
    // NEW: 4-Line Golden Template fields
    judgmentCategory: 'Fundamental Pay Components Rules',
    triggeredCondition: 'Net pay exceeds gross pay',
    whyThisMatters: 'Net pay exceeding gross pay is a mathematical impossibility — taxes and deductions only reduce take-home pay. This will corrupt 941 filings, W-2s, and ACH transmission.',
    reviewerAction: 'Verify gross and net pay columns are not swapped, check for negative tax entries, and correct the source data before re-uploading.',
    uiHints: {
      defaultExpanded: true,
      requiresAcknowledgement: true,
      highlightLevel: 'RED',
